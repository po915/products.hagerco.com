...
export default function NavbarActions(state, dispatch, backend) {
    ...
    const fetchResourcesDropdown = async () => {
        try {
            ...
            const articlesResult = await backend.content.getCategoryArticles(resourcesID);
            for (let i = 0; i < articlesResult.articles.length; i++) {
                const article = articlesResult.articles[i];
    ...
    const fetchAccessControlDropdown = async () => {
        try {
            ...
            const accessIDResult = await backend.content.getCategoryID('Access Control');
    ...
    const fetchProductCategoryDropdown = async (categoryID) => {
        ...
        try {
            ...
            if (!Object.prototype.hasOwnProperty.call(category, 'children')) {
                const categoryItems = await backend.products.getCategoryItems(categoryID);
                buildLinks(categoryItems.result);
            } else if (Object.prototype.hasOwnProperty.call(category, 'children')) {
                const categoryItems = await backend.products.getCategoryDetails(categoryID);
                const sortedItems = categoryItems.children.sort((a, b) => a.name.localeCompare(b.name, undefined, {numeric: true, sensitivity: 'base'}));
                buildLinks(sortedItems);
            }
            dispatch({ type: 'SET_PRODUCT_CATEGORY_DROPDOWN', payload: { categoryID, categoryMenu } });
        } catch (e) {
            dispatch({ type: 'SET_PRODUCT_CATEGORY_DROPDOWN', payload: { categoryID, categoryMenu } });
        }
    };
