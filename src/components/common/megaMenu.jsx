...
export default function MegaMenu() {
    ...
    function mouseOverItem(e) {
        dispatch({ type: 'SET_ACTIVE_SECTION', payload: { section: activeSection } });
    }
    ...
    useEffect(() => {
        if (level === undefined
            ...
            dispatch({ type: 'SET_SECTION_LOADING', payload: { section: activeSection } });
    ...
    if (section.loading || level === undefined || menu === undefined) {
        return <MenuLoading mouseOverItem={mouseOverItem} mouseLeaveItem={mouseLeaveItem} />;
    }
            ...
            <div className="container-fluid">
                <div className="megamenu">
                    <div className="row no-gutters">
                        ...
                        {menu.callout !== undefined
                            ? (
                                <MegaMenuCallout
                                    image={menu.callout.image}
                                    title={menu.callout.title}
                                    text={menu.callout.text}
...
function MegaMenuLinkList({ links, linkCols, level, onLinkClick, setActiveLevel }) {
                    ...
                    <li key={j}>
                        {(() => {
                            switch (link.actionType) {
                                case 'next_level':
                                    return <a href="#" onClick={e => setActiveLevel(e, level + 1, link.id)}>{link.name}</a>;
                                case 'set_preview':
                                    return <a href="#" onClick={e => setActivePreview(e, link)}>{link.name}</a>;
                                default:
                                    return <Link to={link.href} onClick={onLinkClick}>{link.name}</Link>;
    ...
    return (
        ...
        >
            ...
            <div className="megamenu-inner text-light">
                ...
                <p>{text}</p>
                <Link className="btn btn-outline-primary text-light mt-3" to={href}>Learn More</Link>
            </div>
        </div>
    );
}
        ...
        <div className="col">
            <div className="d-inline-flex align-items-start">
                ...
                <div className="megamenu-preview-info ml-4">
                    ...
                    <Link className="btn btn-primary" onClick={onLinkClick} to={href}>{linktext}</Link>
                </div>
            </div>
        </div>
    );
